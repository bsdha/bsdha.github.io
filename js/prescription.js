(function () {
  ...
  let drugList = []; // { brand, generic, form }
  ...
  function renderSuggest(query) {
    ...
    currentSuggestList = drugList.filter((d) => normalize(d.brand).includes(q) || normalize(d.generic).includes(q)).slice(0, 30);
  ...
  ENTER_NAV_FIELDS.forEach((id, fieldIdx) => {
    $(id).addEventListener('keydown', (e) => {
      ...
      // Ô "Tên thương mại" chưa liên kết được với thuốc trong danh sách: khoá con trỏ tại đây khi Enter (chỉ Tab mới cho qua)
      if (id === 'rxFieldBrand' && !brandLinked) {
        return;
      }
  ...
  $('rxPrintBtn').addEventListener('click', async () => {
    if (rxRows.length === 0) { customAlert('Chưa có thuốc', 'Toa thuốc chưa có thuốc nào.'); return; }
